...
phn[, paste("source_name_ch1", collapse = ".")] <- as.character(interaction(as.data.frame(phn[, c("source_name_ch1")])))
...
exprs(sceset)[1:3,1:6] # whats that
counts(sceset)[1:3,1:6] # count sclaed length scaled counts
tpm(sceset) [1:3,1:6] # tpms
#################
# QC 
##################
...
res <- sceset
save(res,file = "~/Desktop/masterthesis/data/sceset_GSE44183-GPL11154.rds")

###### save session info
sink(file = "~/Desktop/masterthesis/results/session_info_GSE44183-GPL11154.txt")
sessionInfo()
sink()
