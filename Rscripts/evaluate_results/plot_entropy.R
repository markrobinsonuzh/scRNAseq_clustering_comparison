...
print( ggplot(data = res_entropy%>%filter( !is.na(s)), aes(x = k, y = s, group=method, color=method))+       
         geom_smooth()+  
...
print( ggplot(data = res_entropy%>%filter( k==truenclust), 
         ...
         manual.scale+
         theme_bw()+
         labs(x="method", y="norm.entropy", title= "normalised entropy per dataset, k=truenclust") 
)
...
print( ggplot(data = res_entropy%>%filter( k==truenclust), 
         ...
         manual.scale+
         theme_bw()+
         labs(x="method", y="norm.entropy", title= "normalised entropy all datasets, k=truenclust" ) 
)
