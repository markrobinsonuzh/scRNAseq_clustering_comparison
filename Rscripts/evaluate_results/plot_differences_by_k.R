...
print(ggplot(diff_abs3, aes(x = method, y = abs(k_diff), group = method, color = method)) + 
        geom_boxplot( outlier.color = NA) + 
        geom_point(position = position_jitter(width = 0.1), alpha = 0.1)+
        theme_bw() +
        scale_color_brewer(palette = "Set3")+
 ...
 ggplot(diff_abs3) +
  geom_boxplot(aes(x = method, y = k_diff, group = method, color = method),
               notch = FALSE, 
               outlier.color = "black")+
                 geom_point(aes(x = method, y = k_diff, group = method, color = method),
