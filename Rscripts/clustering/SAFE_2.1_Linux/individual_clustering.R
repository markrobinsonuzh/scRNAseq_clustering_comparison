...
individual_clustering <- function(inputTags, datatype = "count", 
                            ...
                            tSNE = TRUE, var_genes = NULL, SEED = 1){
  ...
  if(SC3 == TRUE){
    ...
    if (ncol(inputTags) < svm_num_cells){
      exp_cell_exprs <- sc3(exp_cell_exprs, ks = optimal_K, biology = FALSE, gene_filter = gene_filter, rand_seed = SEED)
    }
  ...
  if (Seurat == TRUE){
    ...
    if (length(inputTags[1,]) >= seurat_min_cell){
      ...
      seuratOUTPUT <- FindClusters(object = seuratOUTPUT, reduction.type = "pca", dims.use = 1:nPC, algorithm = 3,
