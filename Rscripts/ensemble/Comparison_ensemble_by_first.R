...
colnames(sum_all2) <- c(
  ...
  "ARIone",
  "truenclust.y")
sum_all3 <- left_join(sum_all2, sum_ensmbl3, by = c("dataset", "methone" = "methone",
                                                    "methtwo" = "methtwo", "run", "k"))
colnames(sum_all3) <- c(
  "dataset",
  "method.ensmbl2",
  "methone",
...
# compute median
res_median <- sum_all %>% dplyr::group_by(dataset, method.ensmbl2, method.ensembl3, 
                                          k, truenclust, methone, methtwo, methtree) %>% 
  dplyr::summarize(ARIensmbl2 = median(ARIensmbl2), 
                   ARIensmbl3 = median(ARIensmbl3),
                   ARIone = median(ARIone), ARIdiff1 = median(ARIdiff1), 
                   ARIdiff2 = median(ARIdiff2))

pdf(gsub("\\.rds$", "_allk.pdf", outrds), width = 12, height = 12)
...
pdf(gsub("\\.rd$", "_truenclust.pdf", outrds), width = 12, height = 12)
