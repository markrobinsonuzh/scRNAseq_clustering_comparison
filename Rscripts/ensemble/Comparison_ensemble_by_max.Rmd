---
title: "Ensemble Clustering by max"
...
# difference 2 based on the max in the ensmble



# check
sum_max <-  sum_all3 %>% group_by( dataset, k) %>% 
  mutate( max.ARI= max(median(ARIone, na.rm=TRUE)) )%>% ungroup()

sum_all <-  sum_max %>% group_by( dataset, methone,k) %>% 
  mutate(ARIdiff1=ARIensmbl2-max.ARI,
         ARIdiff2=ARIensmbl3-max.ARI) %>% 
  ungroup()
res_median<- sum_all
...
Does the consensus with second and third method improve results: 
