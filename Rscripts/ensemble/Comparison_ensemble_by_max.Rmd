...
sum_all <-  sum_all3 %>% group_by( dataset, methone) %>% mutate(ARIdiff1=ARIensmbl2- max(median(ARIone)),
                                                                ARIdiff2=ARIensmbl3-max(median(ARIone)) ) %>% ungroup()
...
res_median <- sum_all%>% group_by(dataset, method.ensmbl2,method.ensembl3, k, truenclust, methone, methtwo, methtree ) %>% 
  dplyr::summarize(ARIensmbl2=median(ARIensmbl2), 
                   ...
                   ARIone=median(ARIone), 
                   ARIdiff1=median(ARIdiff1),
