...
res_ensmbl3 <- readRDS(file="output/ensemble/clustering_ensemble_allmethods3.rds")
res_ensmbl2 <- readRDS(file="output/ensemble/clustering_ensemble_allmethods2.rds")
...
for(i in unique(res_median$dataset) ){ 
  ...
  p2 <- p+ geom_text(aes(color=ARIone , label=round( ARIone ,2) ), size=3, nudge_x = 0, nudge_y = 0, na.rm = TRUE)+
    geom_text(aes(label=round(ARIdiff2,2))) +
    geom_text(aes(label=round(ARIdiff1,2)))+
    geom_nodelab(aes(x=branch), vjust=-.5, size=3)+
    geom_tiplab(aes(x=branch), vjust=-.5, size=3)
  list[[i]] <- p2
  
}
pdf("plots/ensembles/tree_ensemble_dataset3.pdf", width=30,height=30)
lapply(list, print)
dev.off()
