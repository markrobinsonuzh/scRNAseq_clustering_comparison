########################################################
### plot heatmap for differences to max in ari scores ##
########################################################
...
#_______________________________________________________________________
# plot function, plot differences between max ARI per dataset
plot_ari_differences <- function(data, main ) {
  # convert results
  mat <- ari2matrix(data )
  max <- apply(mat,1,max, na.rm = TRUE)
  mat.diff <- mat-max
  # order the datasets based on abs minimum difference
  avg.score <- colMeans(abs(mat.diff))
  new.order <- order(avg.score)
  mat.diff <- mat.diff[, new.order]
  ## plot pheatmap
  pheatmap( mat.diff , color = colorRampPalette(brewer.pal(3, "Greens"))(10),
...
p1 <- plot_ari_differences(files_ari_filtered , main="difference to maximum ARI score: filtered")
p2 <- plot_ari_differences(files_ari_unfiltered, main="difference to maximum ARI score: unfiltered")
p3 <- plot_ari_differences(files_ari_default,  main="difference to maximum ARI score: default" )
#p4 <- plot_ari_differences(files_ari_optimalk, main="difference to maximum ARI score: optimalk" )

p.all <- plot_grid(p1$gtable, p2$gtable, p3$gtable,ncol = 2, nrow=2 ,labels = "auto")
save_plot("results/plots/plot_ari_diff_maxscore_all.pdf", p.all,  base_width=13, base_height  =8)
