...
sceset <- SingleCellExperiment(
  assays = list(counts = cts), colData = phn)

counts(sceset)[1:3,1:3]

assay(sceset, "tpm") <- tpms # tpm is populated with the gene level tpms

assay(sceset, "tpm")[1:3,1:3] # untransformed tpm
assay(sceset, "counts")[1:3,1:3] # untransformed counts
...
save(sceset,file = "data/sceset_raw_GSE52529-GPL16791.rda")
...
is.debris <- colData(sceset)$characteristics_ch1.2=="debris: TRUE" # filter out destroyed cells
is.multiplecell <-  colData(sceset)$characteristics_ch1.4 != "cells in well: 1"
...
```
# Gene filter 
Reduce expression matrix by filtering genes. Filtering is based on mean expression of genes. 
...
expl_vars <- c( "phenoid", "total_features", "source_name_ch1" )
...
```{r plot tSNE} 
set.seed(1234)
out5 <- plotTSNE(sceset.red, exprs_values="normcounts", perplexity=5, colour_by="total_features") + ggtitle("Trapnell 2014")
out5 <- plotTSNE(sceset.red, exprs_values="normcounts", perplexity=5, colour_by="total_features") + ggtitle("Perplexity = 5")
out10 <- plotTSNE(sceset.red, exprs_values="normcounts", perplexity=10, colour_by="total_features") + ggtitle("Perplexity = 10")
out20 <- plotTSNE(sceset.red, exprs_values="normcounts", perplexity=20, colour_by="total_features") + ggtitle("Perplexity = 20") 
