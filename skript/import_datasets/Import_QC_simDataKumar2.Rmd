...
```{r plots}
plotTSNE(sceset, exprs_values = "counts", colour_by="phenoid", perplexity=20)
...
```{r save raw counts}
res <- sceset
save(res,file = "data/sceset_raw_simDataKumar2.rda")
```
...
```{r save data }
res <- sceset
save(res,file = "data/sceset_org_simDataKumar2.rda")

res <- sceset.red
save(res,file = "data/sceset_red_simDataKumar2.rda")
...
```{r tSNE}
p6 <- plotTSNE(sceset.red, exprs_values="logcounts", perplexity=20, colour_by="total_features",size_by="total_counts",return_SCESet=FALSE) + ggtitle("tSNE plot")
```
...
save_plot("results/QC_data/qc_summary_simDataKumar2.png", plot.qc,
