...
cells.Dgcr8 <- phn[, "phenoid" == "Dgcr8 knockout mouse serum+LIF" ]
nrow(cells.Dgcr8)
cells.v6.52iLIF <- phn[, "phenoid" == "v6.5 mouse 2i+LIF" ]
cells.v6.5serumLIF <-  phn[, "phenoid" == "v6.5 mouse serum+LIF" ]
```

# subset the dataset by the populations
...
sceset <- splatSimulate(params, group.prob = c(0.25, 0.5, 0.25), method = "groups",de.prob=c(0.1,0.2,0.4),
                            verbose = FALSE)
...
plotTSNE(sceset, exprs_values = "counts", colour_by="phenoid")
...
NUmber of non expressed features is smaller than in original data
