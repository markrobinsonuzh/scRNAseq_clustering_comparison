...
## sample
b.cells <- b.cells[,sample( colnames(b.cells) , 200, replace = FALSE, prob = NULL) ]
naive.cytotoxic<- naive.cytotoxic[,sample( colnames(naive.cytotoxic) , 200, replace = FALSE, prob = NULL) ]
cd14.monocytes<- cd14.monocytes[,sample( colnames(cd14.monocytes) , 200, replace = FALSE, prob = NULL) ]
regulatory.t <- regulatory.t[,sample( colnames(regulatory.t ) , 200, replace = FALSE, prob = NULL) ]
...
#sceset <- calculateQCMetrics(sceset, feature_controls = list(Mt=grepl("^MT-",  sceset@featureData$symbol) ))


#hist(sceset$pct_counts_feature_controls  , xlab="Mitochondrial proportion (%)", 
...
#mito.med <- median((sceset$pct_counts_feature_controls_Mt), na.rm = TRUE)
#mito.mad <- mad((sceset$pct_counts_feature_controls_Mt), center = size.med, na.rm = TRUE)
#upper.limit.mito <- (mito.med + 3 * mito.mad  )



par(mfrow=c(2,2))
hist(sceset$total_counts, xlab="Library sizes ", main="", 
    breaks=20, col="grey80", ylab="Number of cells")
abline(v=lower.limit.size,col=2, lty=2 )
hist(sceset$total_features, xlab="Number of expressed genes", main="", 
    breaks=20, col="grey80", ylab="Number of cells")
abline(v=lower.limit.feat, col=2, lty=2 )
#hist(sceset$pct_counts_feature_controls_Mt, xlab="Mitochondrial proportion (%)", main="", 
#    breaks=20, col="grey80", ylab="Number of cells")
...
```{r tSNE}
p6 <- plotTSNE(scset.red, exprs_values="normcounts", perplexity=5, colour_by="total_features",size_by="total_counts",return_SCESet=FALSE) + ggtitle("tSNE plot")
```
