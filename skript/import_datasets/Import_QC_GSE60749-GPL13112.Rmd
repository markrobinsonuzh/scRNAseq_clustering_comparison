...
save raw sceset
```{r save raw data, echo=TRUE }
save(sceset,file = "data/sceset_raw_GSE60749-GPL13112.rda")
...
The amount of endogenous should be constant. Cells with a high proportion are removed. Looking at the histogramm the ERCC should be removed regarding the batches.
```{r  filter by spike inn }
hist( sceset$pct_counts_ERCC, xlab="ERCC proportion (%)", 
    ylab="Number of cells", breaks=20, main="", col="grey80" )
    
spike.drop <- isOutlier(sceset$pct_counts_ERCC, nmads=3, type="higher")
```
...
sceset.red <- computeSpikeFactors(sceset.red, general.use=FALSE)
...
```{r some more QC plots} 
p1 <- plotQC(sceset, type = "expl")
p2 <- plotQC(sceset, type = "expl", method = "pairs", theme_size = 6)
p3 <- plotQC(sceset, type = "find-pcs", variable = "total_features",
        plot_type = "pcs-vs-vars")
p4 <- plotQC(sceset, type = "find-pcs", variable = "total_features",
       plot_type = "pairs-pcs")
