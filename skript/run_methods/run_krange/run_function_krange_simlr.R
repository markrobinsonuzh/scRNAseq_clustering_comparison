...
for (i in names(data)){
  ...
  df.clus[,j] <- SIMLR(X = exprs(data[[i]]), c = par.k[[i]][j], no.dim = NA,k=10, if.impute=FALSE, normalize =TRUE, cores.ratio = 1)$y$cluster # use exprs slot of SCeset; log2, normalized count_lstpm
  }
