...
for (i in names(data)){
  require(Rtsne)
    ...
    res.rtsne[[i]] <- Rtsne(X= tinput_matrix[[i]] , perplexity=par.perp[[i]] , pca = TRUE, initial_dims = 20)
...
dir_cluster <- paste0("results/filtered/RtSNEkmeans/RtSNEkmeans_krange_clus_", names(res.cluster), ".txt")
...
dir_systime <-  paste0("results/filtered/RtSNEkmeans/RtSNEkmeans_krange_systime_",names(sys.time),".txt")

save_systemtime(sys.time, dir_systime)

# save experiment labels

file_names <-  paste0("results/filtered/RtSNEkmeans/RtSNEkmeans_krange_labels_",names(labels), ".txt")
...
###### Save Session Info
sink(file = "results/filtered/RtSNEkmeans/session_info_kmeans_krange.txt")
sessionInfo()
sink()
