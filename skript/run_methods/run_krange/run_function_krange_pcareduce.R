...
for (i in 1:(length(input_matrix))){
  input_matrix[[i]] <- scater::exprs(data[[i]]) # use count scaled length scaled tpms, normalized and log2 transformed
}
...
for (i in names(input_matrix)){
    ...
    res.cluster[[i]]  <- pca.red[[i]][, c(par.q[[i]]:n.cluster[[i]]) ] 
    colnames(res.cluster[[i]]) <-  c(paste0( 2:17) ) 
}
  return( res.cluster  )
}
# run PCAreduce
