...
run_cidr <- function(tinput_matrix, par.k , par.nPC){
  require(cidr)
# RUN cidr
...
for  (i in names(sData)) {
    ...
    for ( j in seq_len(length(par.k[[i]])) ){
      df.clus[,j] <- scCluster(object=sData[[i]], nCluster = par.k[[i]][j], cMethod = "ward.D2", nPC=par.nPC[[i]])@clusters# hierarchical clustering on PC , nCluster if user defines cluster number, nPC is the number of PC used for clustering (default is 4), cMethod is hierarchical clustering method default is ward.D2
...
#### run the function
res.cluster <- run_cidr(tinput_matrix, par.k, par.nPC )
