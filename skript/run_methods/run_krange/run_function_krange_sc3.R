...
# run function
run_sc3 <- function( data, par.k, pct_dropout_max ){
...
for (i in names(data)){
    ...
    data[[i]]<- sc3(data[[i]], ks = par.k[[i]],gene_filter=TRUE, pct_dropout_max = pct_dropout_max[[i]]) # perform sc3 clustering
...
res.cluster <-  run_sc3( data, par.k , pct_dropout_max)
