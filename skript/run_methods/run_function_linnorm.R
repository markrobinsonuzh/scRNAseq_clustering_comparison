...
# function 
run_lindworm <- function(data, par.minNonZeroPortion){
...
for (i in names(data)){
    ...
    transformedExp[[i]] <- Linnorm(counts(data[[i]]), spikein=NULL, minNonZeroPortion = 0.25)
...
res.cluster <- run_lindworm( data,par.minNonZeroPortion )
