...
run_function_pcareduce <- function( data, labels, par.nbt, par.q , n.cluster , datatype ){
  ...
  res.cluster <- part <- cl_cons<- sys.time<- input_matrix<- pca.red <- list
  ...
  for (i in names(input_matrix)){
    ...
    sys.time[[i]] <- system.time({
      pca.red[[i]] <- PCAreduce(t(input_matrix[[i]]), nbt = par.nbt[[i]], q = par.q[[i]], method = 'S')
      part[[i]] <- lapply(pca.red[[i]], function(x)x[,par.k(i)])
      part[[i]] <- lapply(part[[i]], function(x) as.cl_partition(x))
      part[[i]] <- as.cl_ensemble(part[[i]])
      cl_cons[[i]] <- cl_consensus(  part[[i]] , method = "SE",  control = list(nruns=50))
      res.cluster[[i]]<- as.character( cl_class_ids( cl_cons[[i]] ) )
